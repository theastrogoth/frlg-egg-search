import { type IVs, type Gender, type SeedInfo } from "./interface";
...
export function getHexString(h: bigint | number, minLength = 4): string {
    let hstr = Number(h).toString(16).toUpperCase();
    for (let i=hstr.length; i<minLength; i++) {
        hstr = '0' + hstr;
    }
    return hstr;
}

export function getSeedString(s: SeedInfo): string {
    return getHexString(s.seed, 4) + ` (${s.delay}ms)`;
}
