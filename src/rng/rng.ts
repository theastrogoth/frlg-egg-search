import { type Nature, type PokemonInfo, type IVs, type Gender, type ShinyType, type Compatibility, type EggMethod, type EggInfo, NATURES } from "./interface";
...
export function getBack(state: bigint): bigint {
    return BigInt.asUintN(16, state);
}
...
export function genderFromPID(pid: bigint, genderThreshold: number): Gender {
    let genderval: number;
    ...
    case 255:
        return "Female";
    default:
        genderval = Number(BigInt.asUintN(8, pid));
        return genderval < genderThreshold ? "Female" : "Male";
    }
...
export function abilityFromPID(pid: bigint): number {
...
export function generatePokemon(states: bigint[], genderThreshold: number, tidxorsid: bigint): PokemonInfo {
    const pid = generatePID(states[0], states[1]);
    const gender = genderFromPID(pid, genderThreshold);
...
export function generatePickupEgg(states: bigint[], heldPID: bigint, parentIVsA: IVs, parentIVsB: IVs, method: EggMethod, genderThreshold: number, tidxorsid: bigint): EggInfo {
...
export function generateEgg(
    ...
    method: EggMethod, genderThreshold: number, tidxorsid: bigint): EggInfo
{
    const heldPID = generateHeldPID(held_states[0], held_states[1], compatibility);
    return generatePickupEgg(pickup_states, heldPID, parentIVsA, parentIVsB, method, genderThreshold, tidxorsid);
}